...
export class MergeEditorModel extends EditorModel {
	...
	public getResultValueWithConflictMarkers(): string {
		...
		for (const [range, state] of states) {
			...
			appendLinesToResult(resultLines, LineRange.fromLineNumbers(resultStartLineNumber, resultRange.startLineNumber));
